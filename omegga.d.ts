...
export type EnvironmentPreset = Preset<"Environment", {
	groups?: {
		Sky?: {
			timeOfDay?: number;
...
export declare type UnrealInteger = number;
export declare type UnrealInteger64 = number;
...
export declare type WirePort = {
	brick_index: number;
	component: string;
	port: string;
};
export declare type Wire = {
	source: WirePort;
	target: WirePort;
};
...
export declare type BrsV14 = Modify<BrsV10, {
	version: 14;
	wires: Wire[];
}>;
export declare type ReadSaveObject = BrsV1 | BrsV2 | BrsV3 | BrsV4 | BrsV8 | BrsV9 | BrsV10 | BrsV14;
...
export type PC<T extends Record<string, unknown> = Record<string, unknown>> = PluginConfig<T>;
export type PS<T extends Record<string, unknown> = Record<string, unknown>> = PluginStore<T>;
...
export interface OmeggaCore {
	 ...
	 * @param worldName World name
	 */
	getWorldRevisions(worldName: string): Promise<{
		index: number;
		date: Date;
		note: string;
	}[]>;
...
export type PluginConfig<T extends Record<string, unknown> = Record<string, unknown>> = T;
/** An omegga plugin */
export declare abstract class OmeggaPlugin<Config extends Record<string, unknown> = Record<string, unknown>, Storage extends Record<string, unknown> = Record<string, unknown>> {
	omegga: OmeggaLike;
...
export type WatcherPattern<T> = (line: string, match: RegExpMatchArray) => T | RegExpMatchArray | "[OMEGGA_WATCHER_DONE]";
